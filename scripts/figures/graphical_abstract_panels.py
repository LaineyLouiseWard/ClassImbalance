...
def scale_bar(arr: np.ndarray, length_m: float = 50.0) -> np.ndarray:
    """A 50 m bar, bottom right, drawn as the manuscript figures draw theirs.

    The abstract's central claim is a DISTANCE -- within 8 m -- so without a bar nothing lets a
    reader judge whether 8 m is one pixel or fifty. A wide white line under a narrow black one,
    matching two_grasslands_qualitative.py and class_seam.py: a halo, not a filled box, so the bar
    sits on the map rather than punching a hole in it.
    """
    out = arr.copy()
    ...
    x1 = w - 26
    x0, yc = x1 - n, h - 30
    out[yc - 5:yc + 5, x0 - 3:x1 + 3] = 255      # white halo
    out[yc - 2:yc + 2, x0:x1] = 20               # the bar
    return out
