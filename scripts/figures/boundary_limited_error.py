...
Boundary-stratified error for the baseline cell: how per-class error concentrates near
ground-truth class boundaries and how much sits deep in field interiors. Three panels:

  (a) Trimap IoU recovery.  Per-class IoU re-scored while excluding a widening band
      around GROUND-TRUTH class boundaries. IoU climbs steeply then plateaus as a
      widening boundary band is excluded. Per-seed mean over 10 seeds (+/-1 SD band),
  ...
  (c) Boundary-band vs interior error, per class.  Paired bars: foreground error rate in
      ...
      Settlement fall to ~0 in the interior (their interior error rate is near zero);
...
Data (all three panels from ONE file, all on the Test A tiles):
  analysis/boundary_error/boundary_trimap_<cell>.json
...
def panel_interior_floor(ax, bvi, use_tex):
    ...
    show rho. Classes whose error is concentrated near boundaries collapse to ~0 in the interior;
    a class with a non-zero interior floor (Seminatural) has genuine interior/semantic error."""
    pc = bvi["per_class"]
...
def trimap_json(root, split, cell):
    ...
    Stage D runs `boundary_trimap_iou.py --out-dir analysis/boundary_error/$SPLIT` for SPLIT in
    {test, external_test}, so the file is PER SPLIT. This script read the un-split path
    `analysis/boundary_error/boundary_trimap_<cell>.json` until 2026-07-27, which stage D never
    ...
    """
    p = root / f"analysis/boundary_error/{split}/boundary_trimap_{cell}.json"
    if not p.exists():
        legacy = root / f"analysis/boundary_error/boundary_trimap_{cell}.json"
