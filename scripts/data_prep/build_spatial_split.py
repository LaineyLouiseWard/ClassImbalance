...
def overlapping_pairs(pool: dict, ids: list[str], buffer_widths: float = 0.0,
                      buffer_m: float | None = None) -> dict:
    ...
    for site, keys in by_site.items():
        ...
        if buffer_m is not None:
            # A buffer stated in metres must be converted into the site's own units, and for a
            # geographic CRS the two axes convert differently -- a degree of longitude at 52N is
            # ~69 km against ~111 km for a degree of latitude. Using one pad for both would make
            # the buffer 60% narrower east-west than intended.
            if abs(float(L[0])) <= 180.0:                      # geographic: degrees
                lat = float(np.mean((B + T) / 2))
                pad_x = buffer_m / (111320.0 * math.cos(math.radians(lat)))
                pad_y = buffer_m / 111132.0
            else:                                              # projected: already metres
                pad_x = pad_y = buffer_m
        else:
            pad_x = pad_y = buffer_widths * w
        for i, ki in enumerate(keys):
            ox = np.minimum(R[i], R) - np.maximum(L[i], L) + pad_x
            oy = np.minimum(T[i], T) - np.maximum(B[i], B) + pad_y
            hit = (ox > tol) & (oy > tol)
            hit[i] = False
...
def propose_with_external(pool: dict, blocks: dict, rng: np.random.Generator,
                          external: tuple) -> dict:
    """Block the splittable sites three ways; hold the external sites out whole.

    The external sites take no part in the block search or the proportion targets -- they are the
    generalisation test set, fixed across every assignment and every seed.
    """
    assign = {t: EXTERNAL for t in pool if site_of(t) in external}
    for site in BLOCK_GRID:
        if site in external:
        ...
        bids = list(per_block)
        rng.shuffle(bids)
        want = {s: TARGET[s] * len(site_tiles) for s in SPLITS}
...
def score(assign: dict, pool: dict, counts: dict, loso: tuple = ()) -> tuple[float, bool]:
    ...
    """
    # External-test tiles sit outside the blocked design, so they must not skew the proportions.
    n = sum(1 for s in assign.values() if s in SPLITS) or 1
    ...
    imbalance = 0.0 if loso else sum(abs(shares[s][c] - pooled[c]) for s in SPLITS for c in pooled)

    # Under LOSO the class prior is deliberately shifted -- the held-out uplands are ~60%
    # semi-natural and carry almost no cropland -- so demanding balanced composition would reject
    # the very design being asked for. Only the floor that guards against a class vanishing applies.
    floor = 0.002 if loso else MIN_CLASS_SHARE
    class_ok = all(shares[s][FOREGROUND[c]] >= floor for s in SPLITS for c in FOREGROUND)
    per_site = Counter((site_of(t), s) for t, s in assign.items())
    external = {site for site in BLOCK_GRID
                if all(assign.get(t) == EXTERNAL for t in assign if site_of(t) == site)}
    if loso:
        ...
        # Sites too small to split are external test; the rest still need all three splits.
        site_ok = all(per_site[(site, s)] >= MIN_SITE_TILES
                      for site in BLOCK_GRID if site not in external for s in SPLITS)
    else:
...
def test_blocks_by_site(assign: dict, blocks: dict) -> dict:
    """{site: frozenset(block ids assigned to test)} — the identity of an assignment's held-out ground."""
    out = defaultdict(set)
    for tid, s in assign.items():
        if s == "test":
            out[site_of(tid)].add(blocks[tid])
    return {site: frozenset(v) for site, v in out.items()}


def is_distinct(assign: dict, blocks: dict, priors: list) -> bool:
    """True if this assignment's test blocks are DISJOINT from every prior one's, at every site.

    Disjoint, not merely unequal: an earlier attempt required only that the sets differ, and the
    inland site still put one block in test under all three assignments, so a third of the inland
    held-out ground was common to every "replicate". Without this, independently seeded assignments
    coincide precisely where it matters most -- a2 and a3 drew identical ireland1 blocks, and two of
    that site's four blocks were train in all three. A spread reported across assignments that share
    held-out ground understates the variability of the priority class, which lives at exactly those
    small sites. Added after the 2026-07-25 audit (S1).
    """
    mine = test_blocks_by_site(assign, blocks)
    for prior in priors:
        if any(mine.get(site, frozenset()) & prior.get(site, frozenset()) for site in mine):
            return False
    return True
...
def main() -> None:
    ...
    ap.add_argument("--buffer-m", type=float, default=None,
                    help="(block designs only) symmetric pairwise buffer in metres")
    ...
    if args.buffer_m is not None:
        print(f"precomputing exclusion graph (buffer = {args.buffer_m:.0f} m) ...", flush=True)
    else:
        print(f"precomputing exclusion graph (buffer = {args.buffer_widths} tile widths) ...",
              flush=True)
    nbr = overlapping_pairs(pool, list(pool), args.buffer_widths, args.buffer_m)

    loso = tuple(args.loso) if args.loso else ()
    external = tuple(args.external_sites) if args.external_sites else ()
    if external:
        unknown = set(external) - set(BLOCK_GRID)
        if unknown:
    ...
    print(f"searching {args.restarts} seeded assignments ...", flush=True)
    best = None
    cand_meta = None
    rng = np.random.default_rng(args.seed)
    tri_gen = three_region_candidates(rng, args.restarts) if args.three_region else iter(())
    for _ in range(args.restarts):
        if args.three_region:
            try:
                axis, fracs, order = next(tri_gen)
            except StopIteration:
                break
            cand = propose_three_region(pool, external, args.three_region, axis, fracs, order)
            cand_meta = {"axis": axis, "fractions": list(fracs), "order": list(order)}
        elif loso:
            cand = propose_loso(pool, blocks, rng, loso)
        elif external:
            cand = propose_with_external(pool, blocks, rng, external)
        else:
            cand = propose(pool, blocks, rng)
        kept, dropped = apply_buffer(cand, nbr)
        if not kept:
            continue
        if priors and not is_distinct(kept, blocks, priors):
            continue
        dev, ok = score(kept, pool, counts, loso)
        # Composition is ranked ABOVE buffer loss. Previously the drop fraction came first, but one
        # tile moves it by ~0.0005 against a 0.0001 rounding granularity, so it almost always broke
        # the tie and the composition term never got a say -- which is how an assignment with 1.3%
        # semi-natural in validation was selected. Losing a few more tiles is much cheaper than a
        # held-out set that misrepresents the priority class. Changed 2026-07-25 (audit S3).
        key = (not ok, round(dev, 4), round(len(dropped) / len(pool), 4))
        if best is None or key < best[0]:
    ...
    geometry = best_meta if args.three_region else None
    if not ok:
        raise SystemExit(
    ...
    manifest = {
        ...
        "three_region_site": args.three_region,
        "external_sites": list(external),
