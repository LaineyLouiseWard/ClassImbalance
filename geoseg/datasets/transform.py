"""
Geometry-aware image/mask transformations for segmentation training.

Includes:
- Random and smart cropping
- Minority-class-aware cropping
- Scale and padding logic

These transforms operate BEFORE Albumentations and ensure
image–mask alignment and class-aware sampling.
"""
...
class RandomCrop(object):
    ...
    def __init__(self, size=512, ignore_index=0, nopad=True):
        if isinstance(size, numbers.Number):
            self.size = (int(size), int(size))  # (H, W)
        else:
            self.size = size  # (H, W)
...
class PadImage(object):
    ...
    def __call__(self, img, mask):
        assert img.size == mask.size
        th, tw = self.size  # FIX: was (self.size, self.size)

        w, h = img.size  # (W, H)

        # If image is wider/taller than target, scale down to fit width then pad
        if w > tw or h > th:
            wpercent = tw / float(w)
            target_h = int(float(h) * float(wpercent))
            img = img.resize((tw, target_h), Image.BICUBIC)
            mask = mask.resize((tw, target_h), Image.NEAREST)
...
class Resize(object):
    def __init__(self, size: tuple = (512, 512)):
        self.size = size  # NOTE: PIL expects (W,H); if square it’s fine
...
class RandomScale(object):
    ...
    def __call__(self, img, mask):
        ...
        if self.mode == "value":
            # FIX: np.random.choice(..., 1) returns array; we need a scalar
            scale_amt = float(np.random.choice(self.scale_list))
        elif self.mode == "range":
            scale_amt = float(random.uniform(self.scale_list[0], self.scale_list[-1]))
        else:
            scale_amt = 1.0
