...
Trains on:
  data/biodiversity_split/train_rep

Sampling:
  WeightedRandomSampler using artifacts/sample_weights.txt

Key change vs Stage 3:
  train_aug_minority (minority-aware cropping)
"""
...
train_dataset = BiodiversityTrainDataset(
    ...
    transform=train_aug_minority,   # ← only change vs Stage 3
)
...
if not sample_weights_path.exists():
    raise FileNotFoundError(
        ...
        "Run: python evaluation/analyze_hard_samples.py"
    )
...
if len(sample_weights) != len(train_dataset):
    raise ValueError(
        f"Weights count ({len(sample_weights)}) does not match train dataset "
        f"({len(train_dataset)})."
    )

print(f"[stage4] Loaded {len(sample_weights)} sample weights from {sample_weights_path}")
