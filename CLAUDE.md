...
- **Datasets:** Biodiversity (proprietary, ~9 GB, not public; ODOS Technologies) + OpenEarthMap
  ...
  chipping meant ~93% of each held-out chip's ground was also in training) and every number produced
...
- **Shipped sampler (minority oversampling, `clsbal` in code):** frequency-only class-balanced
  ...
  **Neither 1.27 nor 2.84 may be quoted as a result** — 1.27 is the target the binary search drives
  ...
  of mean chip sampling weights, so both depend on the chip list they are calibrated against — name it
...
294-chip population rather than the 90-chip subset that is reported. `docs/FINDING_BOUNDARY_IS_PER_CLASS.md`
...
- One inland site cut along one axis: `train | 256 m | val | 768 m | test`, chips straddling either
...
- 256 m is the exact pixel-identity distance (512 px at 0.5 m on a 128 m stride). 650 m was the
  REQUESTED val/test buffer; chips straddling the band are dropped on a 128 m stride, so the REALISED
...
- Chips with no GT boundary are **excluded** from rho: 19 of 191 upland chips (all `ireland2`),
...
1. ~~The teacher confusion matches no split.~~ **RESOLVED 2026-07-26.**
   `artifacts/teacher_oem_gt_confusion_f1.npz` is fitted on 1,072 chips — exactly the training set,
   confirmed by the leakage gate — and `verify_taxonomy_consistency.py` passes 31 checks with no
   drift against the hard-coded `taxonomy.OEM_TO_STUDENT_PRETRAIN`. The untagged 1,846-chip file
...
2. Convergence at 1,072 training chips is assumed, not shown. The budget was set for 1,706. Check the
